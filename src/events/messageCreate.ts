...
import { users, earnedXpMap, bonusMap  } from "../index.js";
...
module.exports = {
    ...
    async execute(message: Message): Promise<void> {
        ...
        if (!xp) { // NOTE: データがない場合は新規作成。
            users.set(message.author.id, 4400);
...
function grantXP(message: Message, xp: number) {
    ...
    const bonusCounter: number | undefined = bonusMap.get(message.author.id); // <-- ボーナスを受け取った回数
